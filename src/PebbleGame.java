...
public class PebbleGame {
    ...
    /**
     * Main Method: Main Thread of the Program.
     * The program's instance of PebbleGame is created here.
     * A user input for the number of players is taken here - must be an integer. Entering 'E'/'e' exits the program.
     * getBagFileLocations(playerNum) is called here.
     * Player objects are created with their corresponding Player Threads.
     * Player move output files are created here.
     * @param args
    ...
    private void writeToFile(Player player, int pebble, Bag bag) {
        ...
        if (bag instanceof BlackBag) {
            //The Bag being a BlackBag means a pebble is being drawn from it
            moveLog = "player" + player.playerNum + " has drawn a " + pebble + " from bag " + bag.getBagName() + "\n";

        }
        else if (bag instanceof WhiteBag) {
            //The Bag being a WhiteBag means a pebble is being discarded to it
    ...
    /**
     * This method changes the PebbleGame's boolean attribute gameWon to true if the combined total size of pebbles in
     * the player's hand is exactly 100.
    ...
    /**
     * This method is called from main() and is used to ask the user for the locations of the files containing the range
     * of values the pebbles in the 3 Black Bags could take. The input is validated to make sure: it is a file, it can
     * be can read and it is not a directory. These are all then passed into readFile() and the results are then passed
     * into the initialiseBags() method along with the number of players in the game.
     * @param playerNum - The number of players in the game (this is just passed into the method initialiseBags())
     */
    private void getBagFileLocations(int playerNum) {
        ...
        //Do-While structures are used here to ensure the inputs are initially asked
        do {
    ...
    private void initialiseBags(ArrayList<Integer> range0, ArrayList<Integer> range1, ArrayList<Integer> range2, int playerNum) {
        ...
        wb0 = new WhiteBag(new ArrayList<Integer>(), "White Bag 0");
        wb1 = new WhiteBag(new ArrayList<Integer>(), "White Bag 1");
        wb2 = new WhiteBag(new ArrayList<Integer>(), "White Bag 2");

        bb0 = new BlackBag(contents0, "Black Bag 0", wb0);
        bb1 = new BlackBag(contents1, "Black Bag 1", wb1);
        bb2 = new BlackBag(contents2, "Black Bag 2", wb2);
    }
